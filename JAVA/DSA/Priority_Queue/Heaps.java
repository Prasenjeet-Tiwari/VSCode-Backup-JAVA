...
public class Heaps {
    ...
    static class Heap {
        ...
        private void heapify(int i) {
            ...
            // (2) if we change the sign here to make it descending order(MaxPriority)
            if (left < arr.size() && arr.get(minIdx) > arr.get(left)) {     //(MinPriority)
                minIdx = left;
            }

            // (3) if we change the sign here to make it descending order(MaxPriority)
            if (right < arr.size() && arr.get(minIdx) > arr.get(right)) {   //(MinPriority)
                minIdx = right;
            }
